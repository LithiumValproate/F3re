...
DB_FILE = 'student_demo_class.sqlite'


class StudentRepository:
    """
    A repository class for handling database operations for Student objects.
    This encapsulates the database connection and queries.
    """
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        """Enable usage with 'with' statements, ensuring connection is closed."""
        if self._conn:
            self._conn.close()
...
def main():
    """Demonstration of class-based repository for database operations."""
    # Create a sample student (same as in the function-based demo)
    teacher = models.Teacher(
        teacher_id=1, name="Dr. Smith", sex=constants.Sex.MALE, department="Computer Science",
        phone=contact.Phone("13800138000"), email=contact.Email("smith@example.com")
    )
    course = models.Course(
        course_id=101, name="Introduction to Programming", teacher=teacher, location="Building A, Room 101",
        credit=3, class_id=frozenset([1, 2]),
        time_slots=(
            models.TimeSlot(
                day=constants.DayOfWeek.MONDAY, start_time=dt.time(9, 0), end_time=dt.time(11, 0),
                repetition=constants.Repetition.WEEKLY
            ),
        )
    )
    student = models.Student(
        student_id=2024001, name="Alice", sex=constants.Sex.FEMALE, birthdate=dt.date(2005, 5, 20),
        enroll_year=2024, major=(1, "Computer Science"), class_id=1,
        phone=contact.Phone("13912345678"), email=contact.Email("alice@example.com"),
        address=models.Address(province="Shanghai", city="Shanghai"),
        family_members=[
            models.FamilyMember(name="Bob", relationship="Father", phone=contact.Phone("13987654321"))
        ],
    ...
    try:
        # Use the repository with a 'with' statement for automatic connection management
        with StudentRepository(DB_FILE) as repo:
            repo.create_tables()
            ...
            retrieved_student = repo.get_by_id(student.student_id)
